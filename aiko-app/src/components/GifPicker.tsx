  ...
  return (
    ...
    >
      <div className="flex justify-between items-center px-1">
        <span className="text-[12px] font-semibold text-[var(--accent)]">Animated Assets</span>
        <button onClick={onClose} className="text-[11px] text-[#5a5248] hover:text-[#f0ebe3] font-medium">Close</button>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {AIKO_ASSETS.map((asset) => (
            <motion.button
              ...
              className="relative group rounded-xl overflow-hidden aspect-video border border-white/5 hover:border-[var(--accent)]/30 transition-all"
            >
              ...
              <div className="absolute inset-0 bg-black/60 flex items-end p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-[11px] font-medium text-[#f0ebe3]">{asset.label}</span>
              </div>
            </motion.button>
          ))}
        </div>
      ...
      <div className="text-[11px] text-[#5a5248] font-medium text-center py-1">
